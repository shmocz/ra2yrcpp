...
void Thread::set_gpr(x86Reg reg, int value) {
#ifdef _WIN32
  auto* c = get_pgpr(reg);
  *c = value;
  save_context(this);
...
int* Thread::get_pgpr(x86Reg reg) {
#ifdef _WIN32
...
Process::Process(u32 pid, u32 perm) : Process(nullptr) {
#ifdef _WIN32
  u32 p = perm;
...
std::string process::proc_basename(std::string name) {
  auto pos = name.rfind("\\");
...
unsigned long process::get_pid(std::string name) {
#ifdef _WIN32
...
void Process::write_memory(void* dest, const void* src, std::size_t size,
                           bool local) {
#ifdef _WIN32
  if (local) {
...
void Process::read_memory(void* dest, const void* src, std::size_t size) {
#ifdef _WIN32
...
void* Process::allocate_memory(size_t size, unsigned long alloc_type,
...
void* Process::allocate_code(size_t size) {
#ifdef _WIN32
...
void Process::suspend_threads(thread_id_t main_tid, duration_t delay) const {
#ifdef _WIN32
...
void Process::suspend_threads(std::vector<thread_id_t> no_suspend,
                              duration_t delay) const {
#ifdef _WIN32
  util::sleep_ms(delay);
...
void Process::resume_threads(thread_id_t main_tid) const {
#ifdef _WIN32
